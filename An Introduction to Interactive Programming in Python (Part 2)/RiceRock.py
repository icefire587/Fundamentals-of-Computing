...
rock_group = set([])
missile_group = set([])
gcol_count = 0 #count missile collide rock
explosion_group = set([])
...
class Sprite:
    ...
    def draw(self, canvas):
        if not self.animated:
            ...
            #explosion animate
            canvas.draw_image(explosion_image, [self.image_center[0]+self.image_size[0]*self.age, self.image_center[1]], self.image_size, self.pos, self.image_size)
    def update(self):
        # update angle
        self.angle += self.angle_vel
    ...
    def collide(self, other_object):
        ...
        radius_2o = self.radius + other_object.get_radius()
        if distance <= radius_2o:
            return True
        else:
            return False
...
def click(pos):
    global started,score,lives,gcol_count
...
def draw(canvas):
    global time, started, rock_group, lives, score, my_ship, missile_group, gcol_count, explosion_group
...
def process_sprite_group(sets, canvas):
    ...
    for x in sets:
        if x.update():
            ...
            x.draw(canvas)
    if len(remove_sets) > 0:
        sets.difference_update(remove_sets)
...
def group_collide(group, other_object):
    ...
    for x in group:
        if x.collide(other_object):
            remove_sets.add(x)
            explosion_group.add (Sprite (x.pos, [0, 0], 0, 0, explosion_image, explosion_info))
            explosion_sound.rewind ()
            explosion_sound.play ()
            col_count += 1
    if len(remove_sets) > 0:
        group.difference_update (remove_sets)
    return col_count        
        
def group_group_collide(sets1, sets2):
    global gcol_count
    remove_sets = set ([])
    for x in sets1:
        if group_collide(sets2, x) > 0:
            remove_sets.add(x)
            gcol_count += 1
#            print gcol_count
    if len (remove_sets) > 0:
        sets1.difference_update(remove_sets)
        sets2.difference_update(remove_sets)
    return gcol_count
...
def rock_spawner():
    ...
    if started:
        ...
        if len(rock_group) < 12:
            ...
            distance = dist(rock.get_position(), my_ship.get_position())
            radius_2o = rock.get_radius() + my_ship.get_radius()
            if distance > radius_2o + 10:
                rock_group.add(rock)
#            else:
#                print distance, radius_2o
